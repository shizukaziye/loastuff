...
(function () {
  ...
  // 4.25 floor; support subtracts the per-CORE order 4.25 floor. Shown as-is — the
  // support coefficients are already per-ally, so this matches the leaderboard (no extra
  // ÷3; the only ×3//3 is in the gold path).
  function gRel(cfg) {
    if (isSupport()) {
      var ov = (A && A.supportOrderValueForCore) ? A.supportOrderValueForCore(A.coreKeyOf ? A.coreKeyOf(cfg) : cfg.coreBase) : null;
      if (A && A.supportDamage && ov != null) return (A.supportDamage(cfg, ov) - 4.25 * ov);
      return supportRelValue(cfg);
    }
...
'  </ul>' +
'  <p><b>Willpower is efficiency, not damage.</b> It <i>reduces</i> the gem’s cost (<code>effective cost = base cost &minus; willpower level</code>), and a cheaper gem of the same damage is strictly better. It enters as a <b>flat credit or tax per effective cost</b>, fitted on hundreds of thousands of simulated roster-bound accounts (cut, equip, fuse &mdash; nothing sold) so grades match which gems actually win a grid slot: costs 3&ndash;4 earn a bonus, 5 is neutral, 6&ndash;9 are taxed harder and harder. Order/Chaos is priced at its exact in-game damage and centered at level 4 (a level-4 order line adds nothing; below subtracts).</p>' +
...
'</div>';
  ...
  function renderLoadout(data) {
    ...
    var sumDmg = gridOk
      ? (sup ? A.gridDamage(valid, "support") : A.gridDamage(valid, "dps"))
